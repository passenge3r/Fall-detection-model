...
- RTMO 的三条路线均未进入前六，最高为 RTMO + ST-GCN++ 的 **78.15% BA**，因此在内部筛选阶段淘汰，不再执行耗时的 MCFD 全量推理。
...
- 共完成 **12 条路线 × 4 折 × 300 轮 = 14,400 epochs**。
...
| 10 | RTMO + PoseC3D-style | 76.34% | 77.65% | 83.54% | 69.14% |
| 11 | YOLO-Pose + PoseC3D-style | 75.69% | 76.65% | 81.01% | 70.37% |
| 12 | RTMO + CTR-GCN | 73.13% | 72.96% | 73.42% | 72.84% |

`PoseC3D-style` 是项目内实现：把归一化骨架渲染为时空热图体，再使用 3D 残差卷积分类。它保留 PoseC3D 的核心表示思想，但不是 MMAction2 官方 SlowOnly-R50 配置，报告中不得写成官方模型复现。

## MCFD 跨数据集固定阈值结果

RTMO 在内部筛选中落后，未进入外测。其余九条路线如下：
...
| PoseC3D | 热图体动作分类 | 项目内 PoseC3D-style 已实现；完成四种姿态前端的内部训练，并纳入九路外测 | 是，但注明 style |
...
| Stacked Hourglass | 姿态骨干 | 官方 MMPose COCO-17 配置和权重地址已确认；当前 Python/PyTorch 环境缺少 mmcv/mmengine 即用运行时，需单独旧环境 | 否 |
