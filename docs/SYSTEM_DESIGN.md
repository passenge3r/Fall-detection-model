...
默认主路线是部署对齐、固定训练 300 轮的 `RTMPose + ST-GCN++`。默认权重位于
...
每帧不再只检查“是否存在任意一个高置信度点”，而是同时检查：

- 至少 5 个有效关节；
- 肩、髋四个躯干点中至少 2 个有效；
- 最长骨骼不超过图像对角线的 35%；
- 人体中心相邻帧跳变不超过图像对角线的 25%。

窗口内通过检查的帧少于 50% 时输出 `UNKNOWN`。默认
`--pose-fallback none`：低质量帧仍保留原后端数值供兼容分类器输入，但决策层拒绝
低质量窗口。`--pose-fallback auto` 会尝试另一姿态后端，仅用于消融实验；在未使用
混合后端骨架重新训练分类器前，不应作为正式系统默认值。
...
| 最少有效关节 | 5 | 排除大面积关键点缺失 |
| 最少躯干关节 | 2 | 排除只有四肢或背景散点 |
| 最大骨骼/图像对角线 | 0.35 | 检测严重错位骨架 |
| 最大中心跳变/图像对角线 | 0.25 | 检测错人和目标切换 |
| 姿态后端回退 | `none` | `auto` 仅作实验，不默认混合骨架 |
| 最少同意模型 | 3/4 折 | 抑制单折高概率造成的误报 |
| 完全无姿态 | `UNKNOWN` | 触发重试或后端回退 |
| 报警冷却 | 10 秒 | 防止重复报警 |
| 证据缓存 | 报警前 5 秒、后 10 秒 | 便于复核 |

这些是系统原型参数，不是最终实验结论。固定 8 段端到端冒烟测试中，
4/4 个跌倒片段被确认，4/4 个 ADL 片段未报警；该小样本测试不是无偏准确率，
设备到位后仍必须基于完整现场验证集调整。
