# API-设备云组件仓-智能门铃-基础功能-低功耗管理-设置功耗模式计划

> 更新时间: 2026-07-06T17:45:27.000+08:00

> 文档ID: 872 | 来源树: OPEN_API
...
   该接口用于设置功耗模式计划。本文档仅适用于设备型号 CS-CMT-CHIME-B，其余型号不保证可用。
...
| Body | plan[].repeatPeriod | Array | 重复周期，为空执行一次，取值范围[1,16] | Y |
...
| Body | plan[].action[].startValue | String | 功耗类型，取值范围[perf,saving,super-saving,non-sleep,normal,user]：perf-性能模式，saving-省电模式，super-saving-超级省电模式，non-sleep-不休眠模式，normal-常规模式，user-用户模式 | Y |

- 请求示例

```
curl --location --request PUT 'https://open.ys7.com/api/v3/otap/prop/{deviceSerial}/Default/0/EnergySaving/EnergyModePlan' \
