# API-设备云组件仓-灯效控制-基础功能-氛围灯控制-设置灯光唤醒

> 更新时间: 2026-07-06T13:49:16.000+08:00

> 文档ID: 1168 | 来源树: OPEN_API
...
   设置灯光唤醒。本文档仅适用于设备型号 CS-HAL-WD2-2C12G，其它型号不保证可用。
...
| body | data | Array | 唤醒计划列表，取值范围[0,] | Y |
| body | data.fade | Integer | 淡入时间 | Y |
| body | data.custom | Boolean | 自定义 | Y |
| body | data.name | String | 唤醒计划名称，取值范围[1,] | Y |
...
| body | data.sustain | Integer | 持续时间 | Y |
| body | data.enabled | Boolean | 使能开关 | Y |

- 请求示例

```
curl --location --request PUT 'https://open.ys7.com/api/v3/otap/prop/{deviceSerial}/Light/1/RGBLightCtrl/WakeUp' \
