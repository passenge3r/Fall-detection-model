...
> 更新时间: 2026-06-24T15:56:59.000+08:00

> 文档ID: 2088 | 来源树: OPEN_API
...
   设置长时间未体动消息提醒计划（MotionLessMsgRemindPlan）
...
| header | propIdentifier | String | 功能点标识，填写报备时的属性标识符，示例值：MotionLessMsgRemindPlan | Y |
| body | plan | Array | 计划列表（范围：[0,8]） | Y |
| body | enabled | Boolean | 定时计划总开关 | N |
| body | plan[].name | String | 计划名称（范围：[0,32]） | N |
| body | plan[].repeatPeriod | Array | 重复周期，为空，执行一次（范围：[1,16]） | Y |
...
--header 'propIdentifier: MotionLessMsgRemindPlan' \
--data-raw '{
    "plan":[
        [
            {
