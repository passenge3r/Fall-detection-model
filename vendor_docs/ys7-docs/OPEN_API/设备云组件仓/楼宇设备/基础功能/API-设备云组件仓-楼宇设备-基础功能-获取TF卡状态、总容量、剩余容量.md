# API-设备云组件仓-楼宇设备-基础功能-获取TF卡状态、总容量、剩余容量

> 更新时间: 2026-07-01T18:44:58.000+08:00

> 文档ID: 1341 | 来源树: OPEN_API
...
   该接口用于获取楼宇可视对讲设备TF卡状态、总容量、剩余容量。本节为楼宇可视对讲透传相关接口，该接口只支持楼宇相关设备。子账户token请求所需最小权限：无。

- 请求地址

`https://open.ys7.com/api/lapp/building/device/storage/status`
...
```
curl --location --request POST 'https://open.ys7.com/api/lapp/building/device/storage/status' \
...
| data.status | Int | TF卡的状态，0表示正常，1表示异常，2表示未插卡，3表示正在格式化 |
| data.total | Int | TF卡总容量，最大支持128G |
| data.free | Int | TF卡剩余容量，0-最大容量 |
