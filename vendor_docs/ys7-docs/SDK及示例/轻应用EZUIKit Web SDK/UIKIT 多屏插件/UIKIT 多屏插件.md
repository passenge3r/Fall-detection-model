...
- 访问GitHub - 轻应用EZUIKit，获取本地插件安装包【CEFBrowserSetup.exe】并完成安装
- 或直接下载插件安装包，并完成安装
- 确保轻应用升级到v8.2.6及以上版本
...
| 参数 | 含义 | 类型 | 示例 | 是否必填 |
| --- | --- | --- | --- | --- |
| deviceList | 设备列表 | array[EZUIKitPlayerParams] | {  id: 'player0',  width: '100%',  height: '100%',  template: 'pcLive',  url: 'ezopen://open.ys7.com/设备序列号/通道号.live',  accessToken: 'at.xxx',  handleSuccess: () => {  console.log('播放成功');  },  handleError: (res) => {  console.error('播放失败', res);  }  } | Y |
...
3. 或者找到插件安装目录下的 kill\_cefsimple.bat 文件，并双击
