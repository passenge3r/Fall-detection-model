# EZOpenSDK-iOS-SDK初始化.md

> EZOpenSDK-iOS-SDK初始化

> 更新时间: 2026-06-02T14:03:45.000+08:00

> 文档ID: 4077 | 来源树: SDK及示例
...
```
[EZOpenSDK initLibWithAppKey:@"Your AppKey" url:@"Your apiUrl" authUrl:nil];
```
...
```
[EZGlobalSDK initLibWithAppKey:@"Your AppKey" url:@"Your apiUrl" authUrl:nil];
```
...
设置的accessToken可能无效或者已过期，授权完成后，需要校验下accessToken的有效性，如果能获取到账号下设备数据，即说明accessToken有效。示例代码如下（具体参考MainViewController.m类实现）
...
```
[EZOpenSDK enableSDKWithTKToken:YES];
[EZOpenSDK initLibWithAppKey:@"Your AppKey"];
```
...
```
[EZOpenSDK setHttpToken:@"Your httpTKToken"];
```
...
- 第一步：调用[EZOpenSDK logout:^(NSError \*error) {}]退出前一个账号；
- 第二步：重新进行SDK初始化：[EZOpenSDK initLibWithAppKey:@"Your new AppKey"];
- 第三步：重新进行授权：[EZOpenSDK setAccessToken:@"Your new accessToken"];
